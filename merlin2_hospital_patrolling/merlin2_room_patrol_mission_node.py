...
class MissionNode(Merlin2FsmMissionNode):
    ...
    def __init__(self):
        super().__init__("mission_mode")

        self.add_state(
            "PREPARING_GOALS",
            CbState([SUCCEED], self.prepare_goals),
            transitions={SUCCEED: "EXECUTING_PATROL"},
        )

        self.add_state(
            "CHECKING_GOALS",
            CbState([SUCCEED], self.check_goals),
            transitions={self.END: SUCCEED, self.HAS_NEXT: "EXECUTING_PATROL"},
        )
